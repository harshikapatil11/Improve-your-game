...
function Auth({ existingProfile, onNew, onDemo }) {
  const [emailOpen, setEmailOpen] = useState(false); const [email, setEmail] = useState(''); const [googleLoading, setGoogleLoading] = useState(false);
  const mockGoogle = () => { setGoogleLoading(true); window.setTimeout(() => onNew(existingProfile || { name: 'Google User', relationshipStatus: 'talking to someone', interactionStyle: 'look for signs', priorities: ['Communication', 'Consistency'], currentGoal: 'understanding what happened', tone: 'bro', email: 'google-user@example.com' }), 700); };
  const continueReturning = () => onNew(existingProfile);
  return <main className="auth-page"><Heart state="healing" /><span className="kicker">A PRIVATE SPACE FOR YOUR STORY</span><h1>Before we start...</h1><p>{googleLoading ? 'Connecting...' : "I should probably know who's telling the story."}</p>{existingProfile ? <button className="auth-primary" onClick={continueReturning}>Continue as {existingProfile.name} <ArrowRight size={16} /></button> : <button className="auth-primary" onClick={() => setEmailOpen(true)}><Mail size={16} /> Continue with Email</button>}<button className="auth-secondary" onClick={mockGoogle} disabled={googleLoading}>{googleLoading ? 'Connecting...' : 'Continue with Google'}</button><button className="demo-link" onClick={onDemo}>Just explore a demo <ArrowRight size={14} /></button>{emailOpen && <form className="email-form" onSubmit={(event) => { event.preventDefault(); onNew({ name: email.split('@')[0] || 'friend', relationshipStatus: 'talking to someone', interactionStyle: 'look for signs', priorities: ['Communication', 'Consistency'], currentGoal: 'understanding what happened', tone: 'bro' }); }}><label>Email for this demo account<input autoFocus value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" type="email" required /></label><button className="auth-primary" type="submit">Continue <ArrowRight size={16} /></button><small>Prototype mode keeps this profile on this device only.</small></form>}</main>;
}
